            ...
            for k in xrange(0,channels):
                if corrImg[(i,j,k)]!=0:
                    ...
                    for couf in range(1, basisNum):  # 1-49
                        phi[:, couf] = mulpdf.pdf(x[(ddMat)], Phi_mu[couf - 1], Phi_sigma[couf - 1]) * 2 * np.pi * np.sqrt(np.linalg.det(Phi_sigma[couf - 1]))
                    ...
                    for couf in xrange(1,basisNum):
                        phi1[couf] = mulpdf.pdf(myposition, Phi_mu[couf - 1], Phi_sigma[couf - 1]) * 2 * np.pi * np.sqrt(np.linalg.det(Phi_sigma[couf - 1]))
