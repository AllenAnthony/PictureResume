...
for side in [16,13,10,7,5]:
    ...
    for testName, noiseRatio in samples.iteritems():
        ...
        minX = np.min(corrImg)
        maxX = np.max(corrImg)
        corrImg = (corrImg - minX) / (
        maxX - minX)  # adjust the value of element to the ratio of the orignal ratio based on max ratio
        ...
        for i in xrange(0, funRowNum):
            for j in xrange(0, funColNum):
                Phi_mu[i, j] = [rowarray[i], colarray[
                    j]]  # mean value of each basis function, divide 1 into baisiNum parts, each ele is the upper-value
        ...
        for i in xrange(0, myrow):
            for j in xrange(0, mycol):
                x[i, j] = [(1.0 * i) / (myrow - 1), (1.0 * j) / (
                mycol - 1)]  # divide 1 into clos-1 parts, and each element stand for the vlaue of segmentation
            ...
            for j in xrange(0, cols):
                for k in xrange(0, channels):
                    if corrImg[(i, j, k)] != 0:
                        ...
                        phi = np.hstack((np.ones((ddNum, 1)), np.zeros(
                            (ddNum, basisNum - 1))))  # the dimension is the normal pixel * number of function
                        for couf in range(1, basisNum):  # 1-49
                            phi[:, couf] = mypdf[couf].pdf(x[(ddMat)]) * 2 * np.pi * np.sqrt(
                                np.linalg.det(Phi_sigma[couf - 1]))
                        ...
                        for couf in xrange(1, basisNum):
                            phi1[couf] = mypdf[couf].pdf(myposition) * 2 * np.pi * np.sqrt(
                                np.linalg.det(Phi_sigma[couf - 1]))
        ...
        prefix = 'result%d/%s_%.1f_%d' % (side, testName, noiseRatio, basisNum)
